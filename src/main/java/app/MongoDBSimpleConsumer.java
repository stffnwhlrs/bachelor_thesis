...
public class MongoDBSimpleConsumer {
    public static void main(String args[]) {
        ...
        long maxReads = 100;
        //String topic = args[1];
        String topic = "ImpactTweetStoreEvents";
        //int partition = Integer.parseInt(args[2]);
        int partition = 0;
        List<String> seeds = new ArrayList<String>();
        //seeds.add(args[3]);
        seeds.add("192.168.178.23");
    ...
    public void run(long a_maxReads, String a_topic, int a_partition,
        ...
        while (a_maxReads > 0) {
            ...
            MongoDatabase db = client.getDatabase("ImpactTweetStore");
            MongoCollection<Document> fishCollection = db.getCollection("ImpactTweetStore");
            Gson gson = new Gson();
            Type type = new TypeToken<ImpactTweetStore>() {}.getType();
            for (MessageAndOffset messageAndOffset : fetchResponse.messageSet(a_topic,
                ...
                payload.get(bytes);
                ImpactTweetStore incomingImpactTweetStore = gson.fromJson(new String(bytes, "UTF-8"), type);
                System.out.println(incomingImpactTweetStore);
                fishCollection.insertOne(incomingImpactTweetStore.getImpactTweetStoreAsDocument());
                numRead++;
                a_maxReads--;
            }
            if (numRead == 0) {
